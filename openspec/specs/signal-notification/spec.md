...
- `title`:含訊號中文名(如「爆量長紅」)與 `as_of` 日期
- `color`:綠色 `0x3ba55d`(爆量是潛在買訊)
- `description`:命中檔數摘要
- `fields`:**每檔一個** field,`name` 為 `{stock_id}`,`value` 為 `量比 {vol_ratio}x · 漲 {red_bar_pct}% · 收 {close}`,依 `severity` 由高到低排序,**最多 10 檔**(Discord field 上限 25,我們留 margin)
...
- **THEN** embed fields 順序為 4.1 → 3.2 → 2.0

#### Scenario: More than 10 hits are truncated

- **GIVEN** 15 檔命中
- **WHEN** embed 構造
- **THEN** fields 只有前 10 檔(severity 最高),title 或 description 註明「+5 more」
